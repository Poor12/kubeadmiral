...
// HasGivenScalarResource checks if Resource has the given scalar resource.
func (r *Resource) HasGivenScalarResource(name corev1.ResourceName) bool {
	if r.ScalarResources == nil {
		return false
	}
...
func GetRelevantResources(su *SchedulingUnit) []corev1.ResourceName {
	resources := make([]corev1.ResourceName, 0, len(DefaultRequestedRatioResources))
	for resourceName := range DefaultRequestedRatioResources {
		switch resourceName {
		case corev1.ResourceCPU:
		...
		case corev1.ResourceMemory:
			resources = append(resources, resourceName)
		default:
			if su.ResourceRequest.HasGivenScalarResource(resourceName) {
				resources = append(resources, resourceName)
			}
		}
	}
